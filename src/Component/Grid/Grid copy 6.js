...
const RtnGrid = (props) => {
  ...
  //#region 헤더 설정
  const setHeader = async (columnId, columnNam, width, parent) => {
    await gfc_sleep(3000);
    ...
    contextFixedUl.onclick = e => {
      ...
      if(frozenCount > index){
        ...
        header.forEach(e => {
          ...
          each.forEach(e => {
            if(e.dataset.columnName === from.name){
              setHeader(from.name, from.header, from.width, e);
              return;
            }
          })
        })

        // const each = header[0].querySelectorAll('th.tui-grid-cell.tui-grid-cell-header');
        // console.log(each)
      }else{
        //1. 해당컬럼 확인
        const from = columns[index];
