...
const CompleteBtn = (props) => {
  ...
  const onProcess = async() => {
    ...
    const msg = `tally_process_erp_procedure.jsp?
                 dScaleNumb=${scaleNumb}&
                 dWorker=${gfs_getStoreValue('USER_REDUCER', 'USER_ID')}&
                 dWorkerName=${gfs_getStoreValue('USER_REDUCER', 'USER_NAM')}&
                 dOutageReasonCode=${detail_subt_leg.getValue() === null ? '' : detail_subt_leg.getValue()}&
                 dOutageWeightCode=${detail_subt.getValue() === null ? '' : detail_subt.getValue()}&
                 dScrapGradeCode=${detail_grade1.getValue()}&
                 dScrapGradeItemCode=${detail_grade2.getValue()}&
                 dTallyHistoryCode=${detail_depr.getValue() === null ? '' : detail_depr.getValue()}&
                 
                 감가비율???

                 dScrapAreaCode=${detail_out.getValue()}&
                 dReturnDivisionCode=${detail_rtn.getValue() === null ? '' : detail_rtn.getValue()}&
