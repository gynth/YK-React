...
function RecImageDtl(props) {
  ...
  const isOpen = useSelector((e) => {
    return e.INSP_CANC_MAIN[props.cam];
  }, (p, n) => {
    return p === n;
  });

  const value = useSelector((e) => {
    return e.INSP_CANC_MAIN.GRID_SCALE;
  }, (p, n) => {
    return p === n;
  });

  // const isActive = useSelector((e) => {
  //   return e.INSP_CANC_MAIN['ON_ACTIVE'];
  // }, (p, n) => {
  ...
  //   const STD_CAM_FOCUS = gfs_getStoreValue('INSP_CANC_MAIN', 'STD_CAM_FOCUS');
  //   const DUM_CAM_FOCUS = gfs_getStoreValue('INSP_CANC_MAIN', 'DUM_CAM_FOCUS');
  ...
  const setModalIsOpen = (open) => {
    ...
    gfs_dispatch('INSP_CANC_MAIN', props.cam, obj);
  }
  const style={
    overlay: {
      position: 'fixed',
